...
const runJava = (content, filePath) =>
  new Promise((resolve) => {
    // Use the exact filename to ensure authentic javac rules apply (e.g. public class matching filename)
    const fileName = filePath ? path.basename(filePath) : "Main.java";
    const classNameToRun = fileName.replace(".java", "");
    ...
    exec(
      ...
      (compileError, compileStdout, compileStderr) => {
        if (compileError) {
          // Compilation failed, do NOT execute. Return authentic javac errors.
        ...
        // 2. Execution Phase
        exec(
          `java -cp "${tempDir}" ${classNameToRun}`,
          { timeout: 30000 },
          (runError, runStdout, runStderr) => {
            ...
            resolve({
              success: !runError,
              output: runStdout,
              error: runStderr || (runError ? (runError.killed ? "Execution timed out (30s)" : runError.message) : null),
            });
          }
        );
      }
...
export const executeLocally = async ({ language, content, path: filePath }) => {
  ...
  if (ext === "java") return runJava(content, filePath);
