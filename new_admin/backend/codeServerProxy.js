...
let hasLoggedSocketHangUp = false;
    ...
    on: {
      error(err, req, res) {
        if (err.message === "socket hang up") {
          if (!hasLoggedSocketHangUp) {
            console.error("[codeServerProxy]", err.message, "target=", req.codeServerTarget, "(repeated socket hang up errors will be hidden)");
            hasLoggedSocketHangUp = true;
          }
        } else {
    ...
    try {
      ...
      const proxy = createProxyMiddleware({
        ...
        pathRewrite: () => {
          ...
          const rewritten = path.slice(idx + "/vscode".length) || "/";
          return qs ? `${rewritten}?${qs}` : rewritten;
        },
        on: {
          error(err, req, socket) {
            // Ignore socket hang up or ECONNREFUSED since code-server reconnects
