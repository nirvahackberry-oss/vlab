  ...
  const proxyMiddleware = createProxyMiddleware({
    ...
    router: (req) => {
      if (!req.jupyterTarget) {
        console.error("[jupyterProxy] Missing jupyterTarget for", req.url);
        throw new Error("Missing jupyterTarget in proxy routing");
      }
    ...
    on: {
      ...
      error(err, req, res) {
        console.error(`[jupyterProxy Error] sessionId=${req.params?.sessionId || 'unknown'} target=${req.jupyterTarget || 'unknown'} error=${err.message}`);
        if (res.writeHead) {
      ...
      proxyRes(proxyRes, req, res) {
        ...
        if (proxyRes.headers.location && req.jupyterProxyBase) {
          ...
          if (loc.startsWith("http")) {
            ...
            proxyRes.headers.location = `${base}/${loc}`;
            console.log(`[jupyterProxy] REWRITTEN NO-SLASH LOCATION: ${loc} -> ${proxyRes.headers.location}`);
          }
        }
    ...
    try {
      ...
      const proxy = createProxyMiddleware({
        ...
        pathRewrite: () => {
          ...
          const rewritten = path.slice(idx + "/jupyter".length) || "/";
          return qs ? `${rewritten}?${qs}` : rewritten;
        },
        on: {
          error(err, req, socket) {
            console.error(`[jupyterProxy WS Error] target=${req.jupyterTarget || 'unknown'} error=${err.message}`);
            // Ignore socket hang up or ECONNREFUSED since frontend reconnects
