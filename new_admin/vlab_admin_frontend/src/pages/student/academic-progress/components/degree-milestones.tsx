...
import { AcademicMilestone } from '@/pages/student/dashboard/types';
import { Award, CheckCircle2, CircleDashed, Clock } from 'lucide-react';
import { Badge } from '@/components/ui/badge';

interface DegreeMilestonesProps {
  milestones: AcademicMilestone[];
}

export function DegreeMilestones({ milestones }: DegreeMilestonesProps) {
  return (
    <Card className="border-border/50 shadow-sm h-full flex flex-col">
      <CardHeader className="pb-4 border-b border-border/40">
        <CardTitle className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
          <Award className="h-5 w-5 text-purple-500" /> Degree Milestones
        </CardTitle>
        <CardDescription className="font-medium mt-1">
          Key achievements in your academic journey
        </CardDescription>
      </CardHeader>
        ...
        <div className="space-y-4">
          {milestones.map((milestone, idx) => (
            <div key={milestone.id} className="relative pl-8 pb-4 last:pb-0">
              ...
              <div className="bg-slate-50 dark:bg-slate-900/50 p-3 rounded-lg border border-slate-100 dark:border-slate-800 ml-2">
